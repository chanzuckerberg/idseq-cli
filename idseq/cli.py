...
def validate_file(path, name):
    ...
    if not re.search(pattern, path):
        print(
            "ERROR: {} ({}) file does not appear to be a fastq or fasta file.".format(
                name, path
            )
        )
...
def main():
    ...
    parser.add_argument(
        "-t", "--token", metavar="token", type=str, help="Your authentication token"
    )
    parser.add_argument(
        "--host-genome-name", metavar="name", type=str, help="Host Genome Name"
    )
    parser.add_argument(
        "--r1",
        metavar="file",
    ...
    parser.add_argument(
        "-b", "--bulk", metavar="file", type=str, help="Input folder for bulk upload"
    )
    parser.add_argument(
        "--starindex", metavar="file", type=str, help="s3 path for STAR index (tar.gz)"
    )
    parser.add_argument(
        "--bowtie2index",
        metavar="file",
    ...
    parser.add_argument(
        "--sampledate", metavar="date", type=str, help="Date of sample collection"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--samplelibrary", metavar="name", type=str, help="Library preparation method"
    )
    parser.add_argument(
        "--samplesequencer", metavar="name", type=str, help="Sequencing instrument"
    )
    parser.add_argument(
        "--samplenotes",
        metavar="name",
    ...
    parser.add_argument(
        "--samplememory", metavar="value", type=int, help="Memory requirement in MB"
    )
    parser.add_argument("--host-id", metavar="value", type=int, help="Host Genome Id")
    parser.add_argument("--job-queue", metavar="name", type=str, help="Job Queue")
    parser.add_argument(
        "--uploadchunksize",
        metavar="value",
        type=int,
    ...
    if not args.bulk:
        if not args.sample_name:
            inp = input(
                "{:35}".format(
                    "\nEnter the sample name (or press Enter to " "use bulk mode): "
                )
            )
            if inp is "":
